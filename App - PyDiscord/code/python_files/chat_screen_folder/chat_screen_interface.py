...
class MessageBox:
    def __init__(self, pos: Sequence[int], size: Sequence[int], picture_path: str, name: str, text: str, time_sent_at: str) -> None:
        ...
        self.time_text = self.fonts["small"].render(
            f"sent at {time_sent_at}", True, LIGHT_GRAY)
        self.collided = False
    ...
    def handle_collision(self) -> bool:
        if self.rect.collidepoint((self.rect.x, 0)) and not self.collided:
            self.collided = True
            return True
        else:
            return False
    ...
    def make_pfp_round(self, size: Sequence[int], pfp) -> None:
        ...
        self.profile_picture.blit(
            mask, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)

    def update_screen_offset(self, value: int):
        self.screen_offset = value

    def draw(self) -> None:
        # Create a nice outline at the bottom of each messagebox
        pygame.draw.rect(self.display_surface, DARKER_GRAY, [
            self.rect.x, self.rect.y + self.screen_offset, self.rect.w, self.rect.h+2], 0, 5)
        pygame.draw.rect(self.display_surface, DARK_GRAY, [
            self.rect.x, self.rect.y + self.screen_offset, self.rect.w, self.rect.h-2], 0, 5)

        # * Bliting onto the screen
        self.display_surface.blit(
            self.profile_picture, (self.rect.x - 65, self.rect.y + self.screen_offset))

        # Blit the name
        self.display_surface.blit(
            self.name, (self.rect.x + 5, self.rect.y + 7.5 + self.screen_offset))

        # Blit the text
        self.display_surface.blit(
            self.text, (self.rect.x+5, self.rect.y+30 + self.screen_offset))

        # Blit the time that the message was sent at
        self.display_surface.blit(
            self.time_text,
            (self.rect.w - self.time_text.get_width()/2 + 15, self.rect.y + self.screen_offset + self.time_text.get_height()/2 + 12))
