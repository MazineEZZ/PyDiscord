...
from log_in_screen_folder.log_in_screen import LogInScreen
from chat_screen_folder.chat_screen import ChatScreen
...
class App():
    ...
    def change_window_info(self, window_size: Sequence[int]) -> None:
        ...
        self.screen = pygame.display.set_mode(
            (window_size["width"], window_size["height"]), flags=self.screen_flag, vsync=self.vsync)
    ...
    def change_screen_info(self) -> None:
        ...
        if self.count == 0:
            ...
            self.log_in_screen = LogInScreen(self.manager, background)
            self.chat_screen = ChatScreen(self.manager)
        else:
            self.log_in_screen.update_manager(self.manager)
            self.chat_screen.update_manager(self.manager)

        self.screen.fill(DARKER_GRAY_BG)

        if self.current_screen == "chat_screen":
            self.object_ids = [
                object_id for object_id in self.chat_screen.object_ids]
            ...
            self.chat_screen.create_text_input()

        elif self.current_screen == "log_in_screen":
            self.object_ids = [
                object_id for object_id in self.log_in_screen.object_ids]

            self.chat_screen.text_input.kill()
            self.log_in_screen.create_all_text_inputs()

        # Initialize the event handler module
        self.handle_events = HandleEvents(self.manager, self.current_screen, self.hwnd,
                                          self.object_ids, self.title_bar, self.log_in_screen, self.chat_screen)
